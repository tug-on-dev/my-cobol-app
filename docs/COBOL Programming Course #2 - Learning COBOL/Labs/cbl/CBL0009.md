...
```
Input File (ACCT-REC):
├── ACCT-NO         PIC X(8)          - Account number
├── ACCT-LIMIT      PIC S9(7)V99 COMP-3 - Account limit (packed decimal)
├── ACCT-BALANCE    PIC S9(7)V99 COMP-3 - Account balance (packed decimal)
├── LAST-NAME       PIC X(20)         - Customer last name
├── FIRST-NAME      PIC X(15)         - Customer first name
├── CLIENT-ADDR                       - Customer address group
│   ├── STREET-ADDR PIC X(25)         - Street address
│   ├── CITY-COUNTY PIC X(20)         - City and county
│   └── USA-STATE   PIC X(15)         - State
├── RESERVED        PIC X(7)          - Reserved space
└── COMMENTS        PIC X(50)         - Account comments

Output File (PRINT-LINE):
└── PRINT-REC       PIC (varies)      - Formatted report record
```

#### Report Layout Structures
```
Header Structures:
├── HEADER-1        - Report title "Financial Report for"
├── HEADER-2        - Date information (Year/Month/Day)
├── HEADER-3        - Column headers (Account, Last Name, Limit, Balance)
...
Control Variables:
└── FLAGS
    └── LASTREC     PIC X VALUE SPACE  - End of file indicator

Accumulator Variables:
└── TLIMIT-TBALANCE
    ├── TLIMITED    PIC S9(9)V99 COMP-3 - Total account limits
    └── TBALANCE    PIC S9(9)V99 COMP-3 - Total account balances

Date/Time Structure:
└── WS-CURRENT-DATE-DATA
    ├── WS-CURRENT-DATE
    │   ├── WS-CURRENT-YEAR     PIC 9(04)
    │   ├── WS-CURRENT-MONTH    PIC 9(02)
    │   └── WS-CURRENT-DAY      PIC 9(02)
    └── WS-CURRENT-TIME
        ├── WS-CURRENT-HOUR     PIC 9(02)
        ├── WS-CURRENT-MINUTE   PIC 9(02)
        ├── WS-CURRENT-SECOND   PIC 9(02)
        └── WS-CURRENT-CENTISECOND PIC 9(02)
```
